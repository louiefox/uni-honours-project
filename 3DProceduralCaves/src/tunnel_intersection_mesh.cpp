...
TunnelIntersectionMesh::TunnelIntersectionMesh(const float angle)
{
	...
	const float LEFT_ANGLE = 90.0f - angle;
	const float RIGHT_ANGLE = 90.0f + angle;
	...
	// Left join
	const glm::vec2& LEFT_TURN_END = getLineEndPos(START_POS, glm::radians(LEFT_ANGLE), 1.0f);
	const glm::vec2& LEFT_TURN_PERPENDICULAR_L = getLineEndPos(LEFT_TURN_END, glm::radians(LEFT_ANGLE - 90.0f), 0.5f);
	...
	// Right join
	const glm::vec2& RIGHT_TURN_END = getLineEndPos(START_POS, glm::radians(RIGHT_ANGLE), 1.0f);
	const glm::vec2& RIGHT_TURN_PERPENDICULAR_R = getLineEndPos(RIGHT_TURN_END, glm::radians(RIGHT_ANGLE + 90.0f), 0.5f);
	...
	// Front join
	const glm::vec2& LEFT_TURN_PERPENDICULAR_R = getLineEndPos(LEFT_TURN_END, glm::radians(LEFT_ANGLE + 90.0f), 0.5f);
	const glm::vec2& RIGHT_TURN_PERPENDICULAR_L = getLineEndPos(RIGHT_TURN_END, glm::radians(RIGHT_ANGLE - 90.0f), 0.5f);
...
glm::vec2 TunnelIntersectionMesh::getLineEndPos(const glm::vec2& startPos, const float radians, const float lineLength)
{
